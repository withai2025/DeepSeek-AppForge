...
**Project Orchestrator** is an Orchestrator-Workers architecture system powered by DeepSeek that turns a single sentence — "I have an app idea" — into a **complete, runnable app**.
...
| 🔧 **Tool Use Scheduling** | Orchestrator dynamically routes sub-agents via the DeepSeek Tool Use mechanism |
...
|-------|------|------|
| 🎛️ Orchestrator | DeepSeek V4 Pro | Read project state, dynamically route sub-agents, manage serial/parallel decisions |
...
ORCHESTRATOR_MODEL=deepseek-v4-pro    # Orchestrator model
WORKER_MODEL_HEAVY=deepseek-v4-pro    # Document generation agent model
WORKER_MODEL_LIGHT=deepseek-v4-flash   # Code execution agent model
...
├── orchestrator.py          # Master scheduler (DeepSeek Tool Use loop)
...
The Orchestrator uses DeepSeek's native Tool Use to make scheduling decisions. It has 4 tools:
...
This system is the companion orchestration system for the [DeepSeek-AppForge](https://github.com/withAIx/DeepSeek-AppForge) Python library:
