...
def show_welcome():
    ...
    console.print(
        Panel(
            ...
            "[bold]Powered by DeepSeek V4[/bold]\n"
...
def setup_api_key():
    """Guide the user to configure their DeepSeek API key.
    ...
    console.print(
        Panel(
            "[bold yellow]⚡ This project needs a DeepSeek API key to work.[/bold yellow]\n\n"
            "The orchestrator calls the DeepSeek API to power 13 AI agents that\n"
            ...
            "  • The key is sent [bold]only to api.deepseek.com[/bold] — nowhere else\n"
    ...
    console.print(
        "\n[bold]To get your key:[/bold] [link=https://platform.deepseek.com/api_keys]https://platform.deepseek.com/api_keys[/link]\n"
    )

    while True:
        key = console.input("[bold]Enter your DeepSeek API key[/bold] (or 'q' to quit): ").strip()
        if key.lower() == "q":
            return False
        if key:
            _write_env_file(key)
...
def _write_env_file(key: str):
    ...
    if env_example.exists():
        ...
        lines = [
            ...
            "WORKER_MODEL_HEAVY=deepseek-v4-pro",
            "WORKER_MODEL_LIGHT=deepseek-v4-flash",
            "PROJECT_NAME=my_app",
        ]
